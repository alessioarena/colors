...
def namedcolor_to_hex(color_name):
    ...
    for n in color_name:
        try:
            out.append(NAMED_COLORS[n.lower()])
        except KeyError:
            raise ValueError("Could not find named color '{0}'".format(n))
...
def namedcolor_to_rgb(color_name):
...
def cmap_to_hex(cmap):
    ...
    try:
        colors = list(map(tuple, cmap.colors))
    except AttributeError:
        colors = list(map(tuple, cmap(np.linspace(0, 1, 256))))
